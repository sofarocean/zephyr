...
static bm_rx_t encoded_rx_buf[2];
...
/* Buffer for received IEE802154 frame payloads */
static uint8_t rx_payload_buf[CONFIG_BM_NUM_FRAMES * CONFIG_BM_MAX_FRAME_SIZE];
        ...
        case UART_RX_RDY:
            ...
            if (pos != old_pos) 
            {
                ...
                if (pos > old_pos) 
                {
                    ...
                    memcpy(encoded_rx_buf[write_buf_idx].buf, &buf[old_pos], pos-old_pos);
                ...
                else
                {
                    ...
                    memcpy(encoded_rx_buf[write_buf_idx].buf, &buf[old_pos], buf_len-old_pos);
                    encoded_rx_buf[write_buf_idx].length = (buf_len-old_pos);
                    if (pos) 
                    {
                        memcpy(&encoded_rx_buf[write_buf_idx].buf[buf_len-old_pos], &buf[0], pos);
