...
static adi_eth_Result_e     MAC_SetForwardMode          (adi_mac_Device_t *hDevice, adi_mac_ForwardMode_t mode, uint32_t mask);
static adi_eth_Result_e     MAC_GetForwardMode          (adi_mac_Device_t *hDevice, adi_mac_ForwardMode_t *pMode);
...
static void macCallback(void *pCBParam, uint32_t Event, void *pArg)
{
    ...
    hDevice->statusRegisters.status0 = status0.VALUE32;
    hDevice->statusRegisters.status1 = status1.VALUE32;
    hDevice->statusRegisters.status0Masked = status0Masked.VALUE32;
    ...
    if (hDevice->statusRegisters.status1Masked & BITM_MAC_STATUS1_P2_PHYINT)
    {
        ...
        hDevice->statusRegisters.p2StatusMasked |= (hDevice->statusRegisters.p2Status & hDevice->phyIrqMask & 0xFFFF0000);
    }
...
adi_eth_Result_e MAC_SetForwardMode(adi_mac_Device_t *hDevice, adi_mac_ForwardMode_t mode, uint32_t mask)
{
    adi_eth_Result_e    result = ADI_ETH_SUCCESS;
...
#endif
    setVal = val | (setVal & mask);

    result = MAC_WriteRegister(hDevice, addr, setVal);
    if (result != ADI_ETH_SUCCESS)
    {
        goto end;
    }
...
adi_eth_Result_e MAC_GetForwardMode(adi_mac_Device_t *hDevice, adi_mac_ForwardMode_t *pMode)
{
    adi_eth_Result_e    result = ADI_ETH_SUCCESS;
    uint16_t            addr = ADDR_MAC_CONFIG2;
    uint32_t            val;
