...
#ifdef CONFIG_PM
static void uart_bm_pm_constraint_set(const struct device *dev)
{
	struct uart_bm_data *data = dev->data;
...
static void uart_bm_pm_constraint_release(const struct device *dev)
{
	struct uart_bm_data *data = dev->data;
...
static inline void uart_bm_set_baudrate(const struct device *dev,
					   uint32_t baud_rate)
{
	const struct uart_bm_config *config = dev->config;
	struct uart_bm_data *data = dev->data;
...
static inline void uart_bm_set_parity(const struct device *dev,
					 uint32_t parity)
{
	const struct uart_bm_config *config = dev->config;
...
static inline uint32_t uart_bm_get_parity(const struct device *dev)
{
	const struct uart_bm_config *config = dev->config;
...
static inline void uart_bm_set_stopbits(const struct device *dev,
					   uint32_t stopbits)
{
	const struct uart_bm_config *config = dev->config;
...
static inline uint32_t uart_bm_get_stopbits(const struct device *dev)
{
	const struct uart_bm_config *config = dev->config;
...
static inline void uart_bm_set_databits(const struct device *dev,
					   uint32_t databits)
{
	const struct uart_bm_config *config = dev->config;
...
static inline uint32_t uart_bm_get_databits(const struct device *dev)
{
	const struct uart_bm_config *config = dev->config;
...
static inline void uart_bm_set_hwctrl(const struct device *dev,
					 uint32_t hwctrl)
{
	const struct uart_bm_config *config = dev->config;
...
static inline uint32_t uart_bm_get_hwctrl(const struct device *dev)
{
	const struct uart_bm_config *config = dev->config;
...
static inline uint32_t uart_bm_cfg2ll_parity(enum uart_config_parity parity)
{
	switch (parity) {
...
static inline enum uart_config_parity uart_bm_ll2cfg_parity(uint32_t parity)
{
	switch (parity) {
...
static inline uint32_t uart_bm_cfg2ll_stopbits(enum uart_config_stop_bits sb)
{
	switch (sb) {
...
static inline enum uart_config_stop_bits uart_bm_ll2cfg_stopbits(uint32_t sb)
{
	switch (sb) {
/* Some MCU's don't support 0.5 stop bits */
#ifdef LL_USART_STOPBITS_0_5
...
static inline uint32_t uart_bm_cfg2ll_databits(enum uart_config_data_bits db,
...
static inline enum uart_config_data_bits uart_bm_ll2cfg_databits(uint32_t db,
								    uint32_t p)
{
	switch (db) {
/* Some MCU's don't support 7B or 9B datawidth */
 ...
 */
static inline uint32_t uart_bm_cfg2ll_hwctrl(enum uart_config_flow_control fc)
{
 ...
 */
static inline enum uart_config_flow_control uart_bm_ll2cfg_hwctrl(uint32_t fc)
{
...
static int uart_bm_configure(const struct device *dev,
				const struct uart_config *cfg)
{
	const struct uart_bm_config *config = dev->config;
	struct uart_bm_data *data = dev->data;
	const uint32_t parity = uart_bm_cfg2ll_parity(cfg->parity);
	const uint32_t stopbits = uart_bm_cfg2ll_stopbits(cfg->stop_bits);
	const uint32_t databits = uart_bm_cfg2ll_databits(cfg->data_bits,
							     cfg->parity);
	const uint32_t flowctrl = uart_bm_cfg2ll_hwctrl(cfg->flow_ctrl);
	...
	if (parity != uart_bm_get_parity(dev)) {
		uart_bm_set_parity(dev, parity);
	}

	if (stopbits != uart_bm_get_stopbits(dev)) {
		uart_bm_set_stopbits(dev, stopbits);
	}

	if (databits != uart_bm_get_databits(dev)) {
		uart_bm_set_databits(dev, databits);
	}

	if (flowctrl != uart_bm_get_hwctrl(dev)) {
		uart_bm_set_hwctrl(dev, flowctrl);
	}

	if (cfg->baudrate != data->baud_rate) {
		uart_bm_set_baudrate(dev, cfg->baudrate);
...
static int uart_bm_config_get(const struct device *dev,
				 struct uart_config *cfg)
{
	struct uart_bm_data *data = dev->data;

	cfg->baudrate = data->baud_rate;
	cfg->parity = uart_bm_ll2cfg_parity(uart_bm_get_parity(dev));
	cfg->stop_bits = uart_bm_ll2cfg_stopbits(
		uart_bm_get_stopbits(dev));
	cfg->data_bits = uart_bm_ll2cfg_databits(
		uart_bm_get_databits(dev), uart_bm_get_parity(dev));
	cfg->flow_ctrl = uart_bm_ll2cfg_hwctrl(
		uart_bm_get_hwctrl(dev));
	return 0;
}
#endif /* CONFIG_UART_USE_RUNTIME_CONFIGURE */

static int uart_bm_poll_in(const struct device *dev, unsigned char *c)
{
	const struct uart_bm_config *config = dev->config;
...
static void uart_bm_poll_out(const struct device *dev,
					unsigned char c)
{
	const struct uart_bm_config *config = dev->config;
#ifdef CONFIG_PM
	 ...
	 * call of uart_bm_irq_tx_[en|dis]able
	 */
...
static int uart_bm_err_check(const struct device *dev)
{
	const struct uart_bm_config *config = dev->config;
	uint32_t err = 0U;
...
static inline void __uart_bm_get_clock(const struct device *dev)
{
	struct uart_bm_data *data = dev->data;
...
static int uart_bm_fifo_fill(const struct device *dev,
...
static int uart_bm_fifo_read(const struct device *dev, uint8_t *rx_data,
				  const int size)
{
	const struct uart_bm_config *config = dev->config;
	uint8_t num_rx = 0U;
...
static void uart_bm_irq_tx_enable(const struct device *dev)
{
	const struct uart_bm_config *config = dev->config;
#ifdef CONFIG_PM
	struct uart_bm_data *data = dev->data;
...
static void uart_bm_irq_tx_disable(const struct device *dev)
{
	const struct uart_bm_config *config = dev->config;
#ifdef CONFIG_PM
	struct uart_bm_data *data = dev->data;
	...
	uart_bm_pm_constraint_release(dev);
#endif
...
static int uart_bm_irq_tx_ready(const struct device *dev)
{
	const struct uart_bm_config *config = dev->config;
...
static int uart_bm_irq_tx_complete(const struct device *dev)
{
	const struct uart_bm_config *config = dev->config;
...
static void uart_bm_irq_rx_enable(const struct device *dev)
{
	const struct uart_bm_config *config = dev->config;
...
static void uart_bm_irq_rx_disable(const struct device *dev)
{
	const struct uart_bm_config *config = dev->config;
...
static int uart_bm_irq_rx_ready(const struct device *dev)
{
	const struct uart_bm_config *config = dev->config;
	/*
	 * On stm32 F4X, F1X, and F2X, the RXNE flag is affected (cleared) by
...
static void uart_bm_irq_err_enable(const struct device *dev)
{
	const struct uart_bm_config *config = dev->config;
...
static void uart_bm_irq_err_disable(const struct device *dev)
{
	const struct uart_bm_config *config = dev->config;
...
static int uart_bm_irq_is_pending(const struct device *dev)
{
	const struct uart_bm_config *config = dev->config;
...
static int uart_bm_irq_update(const struct device *dev)
{
	return 1;
}

static void uart_bm_irq_callback_set(const struct device *dev,
...
static inline void async_user_callback(struct uart_bm_data *data,
...
static inline void async_evt_rx_rdy(struct uart_bm_data *data)
{
...
static inline void async_evt_rx_err(struct uart_bm_data *data, int err_code)
{
...
static inline void async_evt_tx_done(struct uart_bm_data *data)
{
...
static inline void async_evt_tx_abort(struct uart_bm_data *data)
{
...
static inline void async_evt_rx_buf_request(struct uart_bm_data *data)
{
...
static inline void async_evt_rx_buf_release(struct uart_bm_data *data)
{
	struct uart_event evt = {
...
static void uart_bm_dma_rx_flush(const struct device *dev)
{
...
static void uart_bm_isr(const struct device *dev)
{
	struct uart_bm_data *data = dev->data;
...
static int uart_bm_async_callback_set(const struct device *dev,
...
static inline void uart_bm_dma_tx_enable(const struct device *dev)
{
	const struct uart_bm_config *config = dev->config;
...
static inline void uart_bm_dma_tx_disable(const struct device *dev)
{
	const struct uart_bm_config *config = dev->config;
...
static inline void uart_bm_dma_rx_enable(const struct device *dev)
{
	const struct uart_bm_config *config = dev->config;
	struct uart_bm_data *data = dev->data;
...
static inline void uart_bm_dma_rx_disable(const struct device *dev)
{
	struct uart_bm_data *data = dev->data;
...
static int uart_bm_async_rx_disable(const struct device *dev)
{
	const struct uart_bm_config *config = dev->config;
	struct uart_bm_data *data = dev->data;
...
void uart_bm_dma_tx_cb(const struct device *dma_dev, void *user_data,
			       uint32_t channel, int status)
{
	const struct device *uart_dev = user_data;
	struct uart_bm_data *data = uart_dev->data;
	struct dma_status stat;
	unsigned int key = irq_lock();

	/* Disable TX */
	uart_bm_dma_tx_disable(uart_dev);
...
static void uart_bm_dma_replace_buffer(const struct device *dev)
{
	const struct uart_bm_config *config = dev->config;
	struct uart_bm_data *data = dev->data;
...
void uart_bm_dma_rx_cb(const struct device *dma_dev, void *user_data,
			       uint32_t channel, int status)
{
	const struct device *uart_dev = user_data;
	struct uart_bm_data *data = uart_dev->data;
...
static int uart_bm_async_tx(const struct device *dev,
		const uint8_t *tx_data, size_t buf_size, int32_t timeout)
{
	...
	uart_bm_dma_tx_enable(dev);

	return 0;
}

static int uart_bm_async_rx_enable(const struct device *dev,
...
static int uart_bm_async_tx_abort(const struct device *dev)
{
	struct uart_bm_data *data = dev->data;
...
static void uart_bm_async_rx_timeout(struct k_work *work)
{
	struct k_work_delayable *dwork = k_work_delayable_from_work(work);
	struct uart_dma_stream *rx_stream = CONTAINER_OF(dwork,
			struct uart_dma_stream, timeout_work);
	struct uart_bm_data *data = CONTAINER_OF(rx_stream,
			struct uart_bm_data, dma_rx);
	const struct device *dev = data->uart_dev;

	LOG_DBG("rx timeout");

	if (data->dma_rx.counter == data->dma_rx.buffer_length) {
		uart_bm_async_rx_disable(dev);
	} else {
		uart_bm_dma_rx_flush(dev);
	}
}

static void uart_bm_async_tx_timeout(struct k_work *work)
{
	struct k_work_delayable *dwork = k_work_delayable_from_work(work);
	struct uart_dma_stream *tx_stream = CONTAINER_OF(dwork,
			struct uart_dma_stream, timeout_work);
	struct uart_bm_data *data = CONTAINER_OF(tx_stream,
			struct uart_bm_data, dma_tx);
	const struct device *dev = data->uart_dev;

	uart_bm_async_tx_abort(dev);

	LOG_DBG("tx: async timeout");
}

static int uart_bm_async_rx_buf_rsp(const struct device *dev, uint8_t *buf,
				       size_t len)
{
	struct uart_bm_data *data = dev->data;
...
static int uart_bm_async_init(const struct device *dev)
{
	const struct uart_bm_config *config = dev->config;
	struct uart_bm_data *data = dev->data;
...
static const struct uart_driver_api uart_bm_driver_api = {
	.poll_in = uart_bm_poll_in,
	.poll_out = uart_bm_poll_out,
	.err_check = uart_bm_err_check,
#ifdef CONFIG_UART_USE_RUNTIME_CONFIGURE
	.configure = uart_bm_configure,
	.config_get = uart_bm_config_get,
#endif /* CONFIG_UART_USE_RUNTIME_CONFIGURE */
#ifdef CONFIG_UART_INTERRUPT_DRIVEN
	.fifo_fill = uart_bm_fifo_fill,
	.fifo_read = uart_bm_fifo_read,
	.irq_tx_enable = uart_bm_irq_tx_enable,
	.irq_tx_disable = uart_bm_irq_tx_disable,
	.irq_tx_ready = uart_bm_irq_tx_ready,
	.irq_tx_complete = uart_bm_irq_tx_complete,
	.irq_rx_enable = uart_bm_irq_rx_enable,
	.irq_rx_disable = uart_bm_irq_rx_disable,
	.irq_rx_ready = uart_bm_irq_rx_ready,
	.irq_err_enable = uart_bm_irq_err_enable,
	.irq_err_disable = uart_bm_irq_err_disable,
	.irq_is_pending = uart_bm_irq_is_pending,
	.irq_update = uart_bm_irq_update,
	.irq_callback_set = uart_bm_irq_callback_set,
#endif	/* CONFIG_UART_INTERRUPT_DRIVEN */
#ifdef CONFIG_UART_ASYNC_API
	.callback_set = uart_bm_async_callback_set,
	.tx = uart_bm_async_tx,
	.tx_abort = uart_bm_async_tx_abort,
	.rx_enable = uart_bm_async_rx_enable,
	.rx_disable = uart_bm_async_rx_disable,
	.rx_buf_rsp = uart_bm_async_rx_buf_rsp,
#endif  /* CONFIG_UART_ASYNC_API */
};
 ...
 * @return 0
 */
static int uart_bm_init(const struct device *dev)
{
	const struct uart_bm_config *config = dev->config;
	struct uart_bm_data *data = dev->data;
	uint32_t ll_parity;
	...
	if (config->hw_flow_control) {
		uart_bm_set_hwctrl(dev, LL_USART_HWCONTROL_RTS_CTS);
	}

	/* Set the default baudrate */
	uart_bm_set_baudrate(dev, data->baud_rate);
...
#define UART_DMA_CHANNEL_INIT(index, dir, dir_cap, src_dev, dest_dev)	\
	...
	.dma_cfg = {							\
		...
		.dest_burst_length = 1,					\
		.block_count = 1,					\
		.dma_callback = uart_bm_dma_##dir##_cb,		\
	},								\
...
#define BM_UART_IRQ_HANDLER_DECL(index)				\
	static void uart_bm_irq_config_func_##index(const struct device *dev);
#define BM_UART_IRQ_HANDLER(index)					\
static void uart_bm_irq_config_func_##index(const struct device *dev)	\
{									\
	IRQ_CONNECT(DT_INST_IRQN(index),				\
		DT_INST_IRQ(index, priority),				\
		uart_bm_isr, DEVICE_DT_INST_GET(index),		\
		0);							\
...
#define BM_UART_IRQ_HANDLER_FUNC(index)				\
	.irq_config_func = uart_bm_irq_config_func_##index,
#else
...
PINCTRL_DT_INST_DEFINE(index);						\
									\
static const struct uart_bm_config uart_bm_cfg_##index = {	\
	...
	BM_UART_IRQ_HANDLER_FUNC(index)				\
									...
									\
static struct uart_bm_data uart_bm_data_##index = {		\
...
DEVICE_DT_INST_DEFINE(index,						\
		    &uart_bm_init,					\
		    NULL,						\
		    &uart_bm_data_##index, &uart_bm_cfg_##index,	\
